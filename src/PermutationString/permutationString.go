...
func checkInclusionSlidingWindowHashMap(s1 string, s2 string) bool {
	if len(s1) > len(s2) {
		return false
	}
	...
	if maps.Equal(s1Count, s2Count) {
		return true
	}

	l := 0

	for r := len(s1Runes); r < len(s2Runes); r++ {
		if maps.Equal(s1Count, s2Count) {
			return true
		}
		...
		if s2Count[index] == 0 {
			delete(s2Count, index)
		}

		l++
	}

	return maps.Equal(s1Count, s2Count)
...
func main() {
	...
	fmt.Println(checkInclusionSlidingWindowHashMap(s1, s2))
